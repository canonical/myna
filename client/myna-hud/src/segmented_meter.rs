...
mod imp {
    ...
    impl WidgetImpl for SegmentedMeterView {
        ...
        /// segment, lit/unlit by the calibrated level.
        fn snapshot(&self, snapshot: &gtk::Snapshot) {
            ...
            let active = intensity_to_active_segments(intensity, BAR_COUNT);

            let gap = w / BAR_COUNT as f64;
            let bar_width = gap * 0.55;
            for (i, position) in bar_positions().enumerate() {
                let lit = i < active;
                let alpha = if lit { 1.0 } else { 0.16 };
                let color = segment_rgba(position, alpha);
