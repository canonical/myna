...
| Cross-snap data | connect probe `backend` content plug, then `find "$SNAP_DATA/backend"` | only the provider share was exposed: `provider/myna.sock` (mode 666) and `provider/provider.env` (644); no modelctl config/schema data or executable | measured 2026-09-05 with the probe against the retired `run/` share; re-measured 2026-09-14 through the myna snap's `backend` plug connected to `myna-parakeet:provider` |
