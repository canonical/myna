...
Two losses fault `CaptureError::Backend`, and the audio buffered before either still drains ahead of the fault. Graph cycles the data thread misses (an xrun) lose their buffers silently, so it compares the stream clock (`pw_time` ticks) with delivered frames and latches a loss once more than 20 ms is owed; any stream state change (a pause, a relink, a quantum or rate switch) restarts that account. A loop thread starved for longer than the ring holds overflows it; a shorter stall loses nothing. A consumer holding an `AudioStats` borrow stalls the loop thread's publish, so it is subject to the same 2 s bound. Teardown disconnects the stream, which removes the node from the data loop under its lock, before the ring's last audio drains exactly once and the producer finishes.
