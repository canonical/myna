...
| `fetch_funasr_model.py`, `fetch_sherpa_model.py`, `parakeet/fetch_parakeet_onnx.py` | Fetch and stage model weights into a snap directory. Driven by the `snap-*` make targets. |
...
| `stage-qwen-c.sh`, `model-pin.sh`, `lint-packages.sh`, `snap-version.sh` | Snap staging, packaging checks, and the git-derived snap version. |
