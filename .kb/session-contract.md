...
Server ingress ends one of two ways: close (an IE115 commit, `session.finish`, or a malformed IE115 frame after its error is sent) drains audio already accepted then ends the stream; abort (client disconnect, a clean close with no commit, an adapter exit, or server shutdown) discards whatever is queued and releases a blocked reader. The queue bounds the adapter's normalized PCM backlog to 1 MiB per connection regardless of message size - an oversized put is split into whole-frame pieces so nothing waits on room it can never get, and a slow adapter backpressures the reader and, through the socket, the client. websockets' own `max_size` (1 MiB) and `max_queue` (1) bound what sits ahead of that queue.
