...
| T50 | **`myna-audio` skeleton crate**: `AudioSource` impl = `CaptureBackend` (trait) + bounded ring + stats tap; fake backend + unit tests | todo | Charles | T49 | The crate compiles and passes tests with **no PipeWire at all**: a scripted fake `CaptureBackend` (deterministic PCM, fault injection) drives unit tests for press→hold→drain-at-ready, graceful stop (drain then end, §5), abort (drop, discard), overflow policy, fault→one `Err` then end (§4). Ring depth a constructor param (T29 pairing). Honors the invariants: no disk, no content logging, produce exactly the configured `AudioFormat` |
