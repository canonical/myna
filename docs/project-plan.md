...
| T51 | **`pw-record` subprocess backend** (port of `MicSource`) + live verification | todo | Charles | T50 | Spawn/terminate lifecycle, `--target` node selection, whole-frame chunking (~100 ms). Acceptance: `myna-dictate` runs push-to-talk against a live server with the **real mic** through `myna-audio` (replacing `WavFileSource`), exact transcript, zero drops — the Rust equivalent of the `dev/dictate.py` demo, and the first end-to-end exercise of the pre-ready ring on live speech |
