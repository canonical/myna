...
``myna.benchmarker.guard``'s dev-time checks (T02): the same page-fault-thrash
...
``MAJOR_PAGE_FAULT_THRESHOLD`` are the shared primitive: ``myna.benchmarker.guard``
...
mode. Shared with myna.benchmarker.guard's identical-purpose check so the
...
def sample_majflt() -> int:
    ...
    ``myna.benchmarker.guard`` (T02): both need the identical number for the
