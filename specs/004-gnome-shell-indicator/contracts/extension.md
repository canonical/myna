...
| # | Guarantee | Spec |
|---|---|---|
| XH6 | On `enable()` the extension begins watching for the daemon (XH14) and, whenever it is there, spawns `myna-hud` via `Meta.WaylandClient.new_subprocess` and supervises it; on `disable()` it stops watching, terminates the subprocess (or its window), disconnects all signals, and clears all timers (no leaks, no orphans). **(2026-09-01)** Amended: the spawn was unconditional and session-long, which is what XH14 changes. | FR-021, FR-026 |
...
| XH11 | The window is positioned bottom-center of the *active* monitor's work area — the focused window's monitor, else the pointer's, else the primary (`chooseMonitorIndex`) — resolved at adoption and held for that window's life, re-resolved on `monitors-changed`. It follows monitor/work-area changes and the window's own size changes without flicker loops (programmatic moves do not re-trigger themselves). **(2026-08-26)** When an overlay dock (dash-to-dock in auto-hide mode) reserves the bottom of *that* monitor via `Main.layoutManager.dashToDockStruts`, the host shrinks the work area to sit above the dock's reserved extent, so the pill is never covered when the dock slides out; absent the object, placement falls back to the plain work area. | FR-004 |
...
| XH14 | The renderer runs for exactly as long as `com.canonical.Myna.Dictation` is owned. The host follows the shared daemon proxy's `g-name-owner` (`Gio.DBusProxy`, `present-changed` signal, never `AUTO_START` — watching must not be what starts the daemon), spawns on *appeared* and terminates on *vanished*. The proxy is created once and shared with the announcer, so there is a single name watch rather than a per-consumer `bus_watch_name`. A vanish is an expected stop under XH3: no respawn, no dormancy, and a returning daemon resets the restart tally. A respawn already scheduled when the daemon goes away does not fire. **Why the lifetime is not the session's:** the HUD renders that interface and nothing else, so with no daemon it is a process with nothing to draw; and being a snap *app* rather than a snap service it is unreachable by `snap stop myna` while still counting as "running apps", which blocks every `snap install`/`refresh` of its own snap. **(2026-09-01)** | FR-021, FR-026 |
