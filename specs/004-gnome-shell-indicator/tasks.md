...
- [X] T105 [US2] Port `states.js` → `states.rs` + `tests/states.rs` (port of `test/states.test.js`): descriptor `{key, statusText, severity, hidden}`, unknown→ACTIVE tolerance, `notice`/`error` severity split, content-free status formatting. i18n is publisher-owned in the `myna-desktop` gettext domain (R25); the HUD displays `StatusMessage` verbatim. **Test first.** *(Done: 8 ported assertions.)*
...
- [X] T133 [P] i18n move: `client/myna-desktop/po/` (the `myna-desktop` domain; `POTFILES.in` from Rust sources via xgettext), replacing `extensions/myna-shell/po/`. *(Done 2026-08-27; amended 2026-09-01: `client/myna-desktop/po/` — POTFILES.in, regenerated myna-desktop.pot, LINGUAS, README; the publisher resolves lifecycle labels and notification strings through gettext before publishing `StatusMessage`, while `myna-hud` is deliberately untranslated, including its development lab; `MYNA_DESKTOP_LOCALEDIR` overrides the publisher locale root; `make i18n` regenerates the template. Old extension po/ removed.)*
