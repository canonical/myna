...
- [X] T124 `main.rs` modes: default (hosted/standalone — consume `org.myna.Dictation`); parse `--lab`/`--serve-dbus`. Run with GTK on the main thread; the consumer on a worker; channel bridge (the inverse of the old `GtkIndicator` pattern). *(Done 2026-08-26: argv-parsed modes; zbus worker + async-channel bridge.)*
