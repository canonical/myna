...
- **FR-017a** **(2026-08-26)**: The extension MUST make its presence discoverable as a well-known session-bus name (`com.canonical.Myna.Shell`) owned for exactly as long as it is enabled and able to host the indicator. The name carries no properties, methods, or signals — ownership itself is the signal. The desktop client MUST use it to choose the indicator surface: name present → the hosted renderer is the indicator and the client's own fallback indicator is suppressed; name absent → the existing notification fallback applies.
