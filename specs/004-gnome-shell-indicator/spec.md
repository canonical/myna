...
- Q: What happens to the dismiss (×) control if the window takes no input? → A: **Superseded (2026-08-26)** — there is no dismiss control. The indicator takes no pointer input in any state, so its input region is empty always, and a critical error is cleared by the client publishing a new state rather than by the user clicking the pill. The per-state input region this answer described is withdrawn along with FR-007c.
...
- Q: Does the extension still consume `com.canonical.Myna.Dictation`? → A: No. The renderer application becomes the consumer of the existing dictation D-Bus interface (unchanged contract). The extension has no bus surface at all; `myna-desktop` suppresses its notification fallback while any `myna-hud` client is registered via `RegisterClient` and restores it when the last client leaves.
...
- Q: How does a recoverable issue clear, versus a critical error? → A: Recoverable issues auto-dismiss after the same hold window the current indicator already uses (3.5s), during which the user can immediately retry (push-to-talk is unaffected — nothing blocks a new session starting while the pill is still showing). Critical errors never auto-clear on a timer; **(2026-08-26)** they persist until the client publishes a different state, which it does when the condition resolves or the next session starts.
- Q: Is that close control a link to a settings/help surface (as its chevron-like appearance in the reference design might suggest)? → A: **Moot (2026-08-26)** — there is no close control at all. No settings, help, or troubleshooting surface is being designed as part of this change either.
- Q: Does the dismiss button conflict with the indicator's non-focus-stealing invariant (FR-001)? → A: **Moot (2026-08-26)** — the button is gone, so the question cannot arise: nothing in the indicator is pointer-reactive, which is a stronger guarantee than the one this answer offered.
...
- Q: If a second critical error arrives while a first critical-error notice is still showing, what happens? → A: Replace in place — the notice updates to the new error's reason/icon, and its persistence carries over: the replacement still never auto-clears on a timer.
...
A person dictates and nothing was heard (they spoke too quietly, or paused too long before the mic picked anything up) — a brief, self-clearing notice tells them to just try again, and they can start a new session immediately without any extra step. Separately, when something is actually broken (no microphone available, the backend is unreachable), a persistent notice stays on screen with a clear reason, until the client reports that the situation has changed.

**Why this priority**: Not every wire-level `transcription.error` means the same thing to the user. Treating a harmless "didn't catch that" the same as "your microphone is broken" either trains people to ignore real failures (if everything auto-dismisses) or leaves a trivial, self-explanatory miss on screen indefinitely (if nothing auto-dismisses). This distinction is core to the redesign's legibility goal, so it ships with the MVP rather than as a later refinement.

**Independent Test**: Drive `myna-desktop` through (a) a session that finalizes with an empty transcript and (b) a session that hits a hard failure (e.g. simulated microphone-unavailable); assert (a) shows a brief, auto-clearing notice that a new session can start over during or immediately after, and (b) shows a persistent notice that does not clear on its own while the state persists.
...
3. **Given** a hard failure occurs (e.g. no microphone available), **When** that error reaches the indicator, **Then** a persistent notice appears with a clear, content-free reason, and it does NOT clear on a timer while that state persists.
4. **Given** a persistent notice is showing, **When** the client reports a different state (the condition resolved, or a new session begins), **Then** the notice clears and does not reappear on its own. **(2026-08-26)** Previously worded as the person activating a dismiss control; there is no such control.
5. **Given** a persistent notice is showing, **When** the person clicks anywhere on it, **Then** the click reaches the application underneath and keyboard focus does not move (the indicator is click-through in every state — FR-025).
...
- **A critical-error notice is clicked**: the click passes through to the application underneath and keyboard focus does not move — the pill has no interactive control in any state (FR-025).
- **A second critical error arrives while the first is showing**: the notice updates in place to the new reason; it does not stack or queue, and does not become auto-clearing (FR-007d).
...
- **FR-004**: The indicator MUST present as a compact pill positioned bottom-center of the screen — matching the position and general presentation of GNOME's own volume/brightness OSD, not the top-of-panel placement of the prior "goop" design — and MUST render correctly in single- and multi-monitor layouts without appearing off-screen. **(2026-08-26)** The pill is the renderer application's window; the extension positions it bottom-center of the work area of the monitor the user is working on (the focused window's monitor, falling back to the pointer's, then the primary) and re-positions it on monitor/work-area/size changes. **(2026-09-06)** The target monitor is resolved when the window is adopted and held for that window's life, so the pill does not hop between monitors mid-utterance.
...
- **FR-025** **(2026-08-26, amended)**: The indicator MUST be click-through — pointer events MUST pass through it to the application underneath — in **every** state, without exception. It MUST NOT present any interactive control. (The earlier carve-out for a critical-error dismiss control is withdrawn with FR-007b/FR-007c.)
...
- **FR-007**: The indicator MUST distinguish two severities of problem: a **recoverable** issue (e.g. no speech detected in the session) and a **critical** error (e.g. microphone unavailable, backend unreachable). A recoverable issue MUST render as a non-blocking, auto-clearing notice; a critical error MUST render as a persistent notice that does not clear on a timer (see FR-007a–FR-007b). Both MUST surface a clear, content-free reason.
...
- **FR-007b** **(2026-08-26, amended)**: A critical-error notice MUST NOT auto-clear on a timer; it MUST remain visible until the client publishes a different state. It MUST NOT offer a dismiss control — the indicator takes no pointer input (FR-025), and the condition is resolved by the client, not by acknowledging a message.
- **FR-007c** **(2026-08-26)**: *Withdrawn.* There is no dismiss control. FR-025's unconditional click-through supersedes it, and does so more strongly: nothing in the indicator is pointer-reactive at all.
- **FR-007d**: If a new critical error arrives while a critical-error notice is already showing, the notice MUST update in place to the new error's reason (replacing the prior one) rather than stacking or queuing multiple notices; the replacement MUST NOT restart as auto-dismissing.
...
- **FR-010b**: The wave representation MUST be rendered using the user's system accent-color preference as its primary color (with a lighter/highlight tone and a darker/complementary translucent secondary tone derived from it for depth — the darker tone is a computed complement of the primary color, **except when the primary color is orange, where it is a fixed aubergine tone** rather than a generic computed complement), and MUST fall back to a fixed default color (Ubuntu orange) only when no accent color can be resolved from the system at all. **(Amended 2026-08-26)** The primary color is whatever accent the desktop is *actually using*, as reported by the platform's own theme; the indicator MUST NOT second-guess it by asking whether the user "actively chose" it. The prior rule — an untouched system default must be treated as not actively chosen — is withdrawn: it rested on the premise that an untouched Ubuntu desktop reports `'blue'` while appearing orange, which is false (`ubuntu-settings` ships a settings override making the effective default `'orange'`), and it produced a wrong result on desktops using a Yaru accent variant, re-tinting a visibly olive desktop back to orange.
...
- **FR-017a** **Fallback suppression now uses the `RegisterClient` client set on `com.canonical.Myna.Dictation` — see `contracts/dbus-interface.md` C14/C15.)**
...
- **FR-020**: The extension MUST declare the GNOME Shell versions it supports (GNOME 46–51) and MUST NOT attempt to load on unsupported Shell versions. Where Mutter moves an API within that range, the host MUST capability-detect and use the API generation the running Shell exposes.
- **FR-021**: The extension MUST re-initialize cleanly across Shell restart/session relogin and MUST release all actors, timers, subprocesses, and D-Bus subscriptions on disable (no leaks). **(2026-08-26)** now includes terminating the hosted renderer process.
...
- **Shell presence** **(Removed: `com.canonical.Myna.Shell` no longer exposed; see FR-017a.)**
- **Hosted overlay window** **(2026-08-26)**: the renderer application's toplevel window while hosted on GNOME — dock-typed, hidden from window lists, on all workspaces, always above normal windows, click-through except for the critical-error dismiss control, positioned bottom-center of the active monitor's work area by the extension.
...
- **SC-008**: The extension loads and functions on the targeted GNOME Shell versions (GNOME 46–51) and refuses to load on unsupported versions rather than failing at runtime.
...
- **D-Bus contract owned here, implemented in `myna-desktop`**: this feature defines the session-bus interface (state + state-change signal + audio-level values + error severity + optional start/stop/toggle + error message, plus `RegisterClient`/`UnregisterClient`); the emitting side is added to `myna-desktop`. Exact member names/signatures are a design detail resolved in planning, guided by the landscape's `com.canonical.Myna.Dictation` sketch.
...
- **Target platform**: Ubuntu Desktop on Wayland with GNOME 46–51; older GNOME and non-GNOME desktops are out of scope.
...
- **Accent-color and reduced-motion are desktop-environment preferences, not new settings this feature introduces**: both are read from GNOME's existing system-wide preferences. **(2026-08-26)** both are now sourced inside the renderer application via the platform's own machinery, since the application no longer runs inside the Shell: the accent is read from the *theme* (a widget styled `color: @accent_bg_color`), which reports the colour the desktop is actually using and needs no version probing, with the libadwaita style manager and the interface settings as fallbacks. The FR-010b untouched-default fallback rule is **withdrawn** — see FR-010b.
...
- **A developer lab and simulator ship inside the renderer binary** **(2026-08-26, supersedes the prior standalone dev-lab assumption)**: lab mode (manual controls + dictation target, no backend required) and simulator mode (serves `com.canonical.Myna.Dictation`) are modes of `myna-hud` driving the identical renderer modules. They are development aids, not user-facing surfaces, and carry no independent functional requirements beyond behaving identically to the shipped rendering paths.
