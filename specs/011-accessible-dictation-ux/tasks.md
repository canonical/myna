...
- [X] T025 Implement the GJS-side loader/check in a small pure function within `extensions/myna-shell/a11y.js` (or a sibling module). Satisfies T024. *(Implemented as a sibling `coverage.js` module to keep `a11y.js` focused on announcements. It now lives at `extensions/myna-shell/test/coverage.js`: its only consumer is `test/coverage.test.js`, and a module the shipped bundle never imports has no business at the extension root — the bundle review gate flags exactly that.)*
...
- [X] T080 [P] Update `docs/desktop-injection.md` / `docs/project-plan.md` to reflect the new `accessibility`/`sound`/`failure`/`coverage` modules and close out T56; annotate T31/T54/T58/T59/T61/T62 with this feature's overlap resolution (per spec Assumptions) *(Target void: neither file is in the tree, and no commit on this branch touches `docs/` — `docs/` was restructured upstream into the `.kb/` knowledge base, which does not carry the T-numbers this task refers to. The tick is left as found rather than flipped, because what was or was not written into a document that no longer exists cannot be verified from here. The module summary this task would have produced is instead carried by the modules' own doc comments and by research.md/plan.md.)*. *(`docs/desktop-injection.md` gained an "Accessibility" section summarizing the four new seams and how `myna-cli` shares the `failure` registry across the crate boundary. `docs/project-plan.md`: T56 closed out (done, feature 011) with a summary across all five user stories; T58/T59/T62/T31/T54 each annotated with what this feature did and did NOT resolve for them (verified via direct code inspection — e.g. T59 is now genuinely resolved upstream rather than left open: the rebase onto `integration-220627` brought `myna_desktop::AutoStop`, which consumes the `silence-timeout` key end to end, superseding this feature's persisted-but-unconsumed `silence_auto_stop_seconds` preference).)*
